...
function Write_File($content, $server_file, $ServerUID, $mode)
{
    global $con;
    $rand1      = base64_encode(md5(base64_encode((rand(1, rand(1, 9999999999) * rand(1, rand(1, 9999999999)))))));
    $rand       = substr($rand1, 0, -1);
...
function Upload_File_FTP($remote_file, $filepath, $ftp_server, $ftp_user_name, $ftp_user_pass)
{
    global $con;
...
if (ftp_put($conn_id, $remote_file, $filepath, FTP_BINARY)) {
 ...
 return "There was a problem while uploading $filepath\n" . $error_get_last['message'];
}
