...
function CORE_Check_Force_SSL(){
    ...
    if($config['ManageMC_URL_Checks'] == 'TRUE') {
	//Check that we are on the correct domain
	if ($config['ManageMC_Domain'] == $_SERVER['HTTP_HOST']) {return true;}
	else{header('location: http://' . $config['ManageMC_Domain'] . $_SERVER['REQUEST_URI'] . '');}
    }
}
...
function CORE_Render_Sidebar(){
    ...
    echo '<!-- /START CORE_Render_Sidebar()/ -->
	...
	<div class="col-md-2" style="padding-left:25px; background-color: #FFF;">
		...
		echo '<hr style="margin-top: -10px; margin-bottom: -10px;" /><ul class="nav nav-sidebar"><h3>Servers</h3>';
		...
		if ($_SERVER["SCRIPT_NAME"] == "/server_management.php" && $page !== "/server_management.php?page=AddServer" && $_SERVER['REQUEST_URI'] !== "/server_management.php"){
...
function CORE_Render_Navbar(){
	...
	<div class="container-fluid">
		...
		<small class="navbar-brand" style="font-size: 14px;">'.$ManageMC_Version.'</small>
	    </div>
